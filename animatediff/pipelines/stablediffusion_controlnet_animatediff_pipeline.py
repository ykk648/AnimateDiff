...
class StableDiffusionControlNetAnimateDiffPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                down_block_res_samples, mid_block_res_sample = self.controlnet(
                    control_model_input.to(controlnet.dtype),
                    t,
                ...
                # Broad to (b f) c h w
                zero_padding = video_length-1
                down_block_res_samples = [torch.cat([d[0].unsqueeze(0)] + [torch.zeros_like(d[0].unsqueeze(0))] * zero_padding + [d[1].unsqueeze(0)] + [torch.zeros_like(d[1].unsqueeze(0))] * zero_padding, dim=0) for d in down_block_res_samples]
                zero_dummy_m = torch.zeros_like(mid_block_res_sample[0].unsqueeze(0))
                mid_block_res_sample = torch.cat([mid_block_res_sample[0].unsqueeze(0)] + [zero_dummy_m] * zero_padding + [mid_block_res_sample[1].unsqueeze(0)] + [zero_dummy_m] * zero_padding, dim=0)
                # down_block_res_samples = [torch.cat([d[0].repeat(video_length,1,1,1), d[1].repeat(video_length,1,1,1)], dim=0) for d in down_block_res_samples]
                # mid_block_res_sample = torch.cat([mid_block_res_sample[0].repeat(video_length,1,1,1), mid_block_res_sample[1].repeat(video_length,1,1,1)], dim=0)
